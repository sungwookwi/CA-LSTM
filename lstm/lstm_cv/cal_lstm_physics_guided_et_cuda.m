...
for i = 1:length(basin_list)   
    ...
    %basin_et_train(basin_et_train(:,4)<0,4) = nan;
    %basin_et_test(basin_et_test(:,4)<0,4) = nan;
...
for i = 1:length(basin_list)
    ...
    for j = 1:size(q_basin,1)
        ...
        if ~isnan(q_basin(j,end))
            ...
            XTrain{n} = x_seq;
            YTrain{n} = [q_basin(j,end);et_basin(j,end);];
...
%         Yobs = single(reshape([YTrain_mini{:}],2,minibatch_size));


        X = gpuArray(X_all);
        Yobs = single(gpuArray(reshape([YTrain_mini{:}],2,minibatch_size)));
        ...
        Loss_1st = w_q(www)*L_mse_batch;
        Loss_2nd = w_et(www)*L_mse_et_batch;
        ...
        dy_q = w_q(www)*(Yhat(1,:)-Yobs(1,:)).*d_relu;
        dy_et = w_et(www)*(Yhat(2,:)-Yobs(2,:));
        
        dV = [dy_q;dy_et]*H_end';
        dbv = [sum(dy_q);sum(dy_et)];
...
save(['hybrid_lstm_',hydro_sel,'_ETout_epoch',num2str(num_epoch),'_drop',num2str(dropout_rate*100),'_alpha',alpha_str{www},'_trial',num2str(trial),'.mat'],...
    'U_final','W_final','V_final','b_final','bv_final',...
    'loss_calib_epoch','loss_calib','loss_calib_1st_epoch','loss_calib_1st','loss_calib_2nd_epoch','loss_calib_2nd',...
