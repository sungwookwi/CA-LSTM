...
hydro_model = {'sacsma','hymod','vic'};
%hydro_model = {'hymod','vic'};
...
% ET for each basin
datemat_et = datevec(datenum([1987 10 1]):datenum([2013 9 30]));
datemat_et_train = datevec(datenum([1988 1 1]):datenum([2003 12 31]));

basin_et_train_all = nan(size(datemat_livneh_train,1),length(basin_list));
for i = 1:length(basin_list)
    
    basin_id = basin_list{i};
    
    %basin_et_hydromodel = load(['./basin_et_',hydro_sel,'/trial_1/simaet_',hydro_sel,'_',basin_id,'.txt']);
    basin_et_hydromodel = load(['/home/ca-lstm/hydromodel/',hydro_sel,'/simaet_watershed/simaet_',hydro_sel,'_',basin_id,'.txt']);
    basin_et = basin_et_hydromodel(:,4);
    
    sind_train = find(datemat_et(:,1)==syr_train & datemat_et(:,2)==1 & datemat_et(:,3)==1);
    eind_train = find(datemat_et(:,1)==eyr_train & datemat_et(:,2)==12 & datemat_et(:,3)==31);
    %sind_train = find(datemat_et(:,1)==syr_train-1 & datemat_et(:,2)==10 & datemat_et(:,3)==1);
    %eind_train = find(datemat_et(:,1)==eyr_train & datemat_et(:,2)==9 & datemat_et(:,3)==30);
    sind_test = find(datemat_et(:,1)==syr_test & datemat_et(:,2)==1 & datemat_et(:,3)==1);
    eind_test = find(datemat_et(:,1)==eyr_test & datemat_et(:,2)==12 & datemat_et(:,3)==31);         
    
    basin_et_train = basin_et(sind_train:eind_train);
    
    basin_et_test = basin_et(sind_test:eind_test);

    
    basin_et_train_all(:,i) = basin_et_train;
...
basin_et_train_all_mean = mean(basin_et_train_all(:));
basin_et_train_all_std = std(basin_et_train_all(:),1);

clear('basin_et_train_all')
...
for i = 1:length(basin_list)
    ...
    eval(['basin_et_train = et_train_',basin_list{i},';'])
    eval(['et_standard_train_',basin_list{i},' = (basin_et_train-basin_et_train_all_mean)/basin_et_train_all_std;'])
    eval(['clear(''et_train_',basin_list{i},''')'])
    eval(['basin_et_test = et_test_',basin_list{i},';'])
    eval(['et_standard_test_',basin_list{i},' = (basin_et_test-basin_et_train_all_mean)/basin_et_train_all_std;'])
    eval(['clear(''et_test_',basin_list{i},''')'])
...
for i = 1:length(basin_list)
    ...
    eval(['et_basin = et_standard_train_',basin_id,';'])
    ...
    for j = 1:size(q_basin,1)
        ...
        if ~isnan(q_basin(j,end))
            ...
            for tt = meteo_ind-seq_length+1:meteo_ind
                x_seq(:,nn) = [pr_basin(tt);tmax_basin(tt);tmin_basin(tt);daylen_basin(tt);et_basin(tt);...                   
                    ];
                nn=nn+1;
            end
            XTrain{n} = x_seq;
...
for i = 1:length(basin_list)   
    ...
    eval(['clear(''et_standard_train_',basin_id,''')'])
...
%         Yobs = single(reshape([YTrain_mini{:}],1,minibatch_size));

        X = gpuArray(X_all);
        Yobs = single(gpuArray(reshape([YTrain_mini{:}],1,minibatch_size)));
        ...
        %Yhat_q = Yhat(1,:);
        %L_nonzero = Yhat_q(Yhat_q<0)*-1;
        %L_nonzero_batch = sum(L_nonzero)/minibatch_size;
...
save(['hybrid_lstm_',hydro_sel,'_ETin_epoch',num2str(num_epoch),'_drop',num2str(dropout_rate*100),'_trial',num2str(trial),'.mat'],...
    'U_final','W_final','V_final','b_final','bv_final',...
    'loss_calib_epoch','loss_calib',...
