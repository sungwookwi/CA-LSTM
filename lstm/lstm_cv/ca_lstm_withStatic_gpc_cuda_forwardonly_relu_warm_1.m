...
warm_yr = [1990 1992 1994 1996 1997 2000 2001 2002 2003 2004 2006 2007 2009 2012 2013];

for dr = 3
    
    dropout_rate = dr_vec(dr);
...
datemat_livneh_train = datevec(datenum([1988 1 1]):datenum([2013 12 31]));
datemat_livneh_train_wy = datemat_livneh_train(:,1:3);
datemat_livneh_train_wy(datemat_livneh_train(:,2)>=10,1) = datemat_livneh_train(datemat_livneh_train(:,2)>=10,1)+1;
datemat_livneh_train_wy_warm = datemat_livneh_train_wy;
datemat_livneh_train_wy_warm(~ismember(datemat_livneh_train_wy(:,1),warm_yr),:) = [];

basin_pr_train_all = nan(size(datemat_livneh_train_wy_warm,1),length(basin_list));
basin_tmax_train_all = nan(size(datemat_livneh_train_wy_warm,1),length(basin_list));
basin_tmin_train_all = nan(size(datemat_livneh_train_wy_warm,1),length(basin_list));
basin_daylen_train_all = nan(size(datemat_livneh_train_wy_warm,1),length(basin_list));
...
%     basin_climate_livneh = load(['G:\CA-LSTM\meteo\livneh/meteo_livneh_CDEC_watershed_',basin_id,'_fix.txt']);
%     basin_daylen = load(['G:\CA-LSTM\LenDayLight/daylight_CDEC_watershed_',basin_id,'.txt']);
    ...
    datemat_livneh_wy = datemat_livneh(:,1:3);
    datemat_livneh_wy(datemat_livneh(:,2)>=10,1) = datemat_livneh(datemat_livneh(:,2)>=10,1)+1;
    ...
    basin_pr_train_warm = basin_pr(ismember(datemat_livneh_wy(:,1),warm_yr));
    basin_tmax_train_warm = basin_tmax(ismember(datemat_livneh_wy(:,1),warm_yr));
    basin_tmin_train_warm = basin_tmin(ismember(datemat_livneh_wy(:,1),warm_yr));
    basin_daylen_train_warm = basin_daylen(ismember(datemat_livneh_wy(:,1),warm_yr));

    


    
    basin_pr_train_all(:,i) = basin_pr_train_warm;
    basin_tmax_train_all(:,i) = basin_tmax_train_warm;
    basin_tmin_train_all(:,i) = basin_tmin_train_warm;
    basin_daylen_train_all(:,i) = basin_daylen_train_warm;
...
% cdec_basin_static_feature = xlsread('G:\CA-LSTM\static_feature/CatchmentStaticFeature.xlsx');
...
datemat_train = datevec(datenum([1988 10 1]):datenum([2013 9 30]));
datemat_train_wy = datemat_train(:,1:3);
datemat_train_wy(datemat_train(:,2)>=10,1) = datemat_train(datemat_train(:,2)>=10,1)+1;
datemat_train_wy_warm = datemat_train_wy;
datemat_train_wy_warm(~ismember(datemat_train_wy(:,1),warm_yr),:) = [];
...
%     basin_data = load(['G:\CA-LSTM\CDEC\FNF_mm_QC/FNF_',basin_id,'_mm.txt']); 
    basin_datemat = basin_data(:,1:3);
    basin_datemat_wy = basin_datemat;
    basin_datemat_wy(basin_datemat(:,2)>=10,1) = basin_datemat(basin_datemat(:,2)>=10,1)+1;
        
    
    basin_q_train = basin_data(ismember(basin_datemat_wy(:,1),warm_yr),:);
...
% Dropout rate
%dropout_rate = dr_vec(dr); % 0 is no dropout node; 0.2: on average 20% of hidden nodes are dropped out
...
% tic;
	...
	Yhat(Yhat<0) = 0; % ReLU activation function 
        	Loss = 1/2*(Yhat-Yobs).^2;
        ...
        batch_info_Yhat(1:minibatch_size,n) = Yhat;
        batch_info_Yobs(1:minibatch_size,n) = Yobs;
...
% toc;
figure
plot(loss_calib,'b'); hold on;
xlabel('Number of Iteration')
ylabel('Loss Function')
title('LSTM-PUB Loss Function Convergence')
grid on;
...
figure
plot(loss_calib_epoch,'b'); hold on;
...
save(['ca_lstm_train_result_h',num2str(num_hiddn_lyr_unit_1),'_e',num2str(num_epoch),'_b',num2str(batch_size),'_drop',num2str(dropout_rate*100),'_',loss_type,'_',num2str(kk),'_relu_warm.mat'],'U_final','W_final','V_final','b_final','bv_final','loss_calib_epoch','loss_calib','batch_info_selbasin','batch_info_datenum','batch_info_Yhat','batch_info_Yobs');
